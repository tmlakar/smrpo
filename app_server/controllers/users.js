...
const prikaziStran = (req, res, uporabniki) => {
    ...
    console.log("dobim cookie delaaaa", req.cookies.authcookie)
        //dodan nov uporabnik
    var successfullyCreateUser = req.query.addUser;
    var uspesnoDodanUporabnik = false;
    if (successfullyCreateUser == "successfullyAddedUser") {
        uspesnoDodanUporabnik = true;
    }
    //napaka dodajanje novega uporabnika
    var neuspesnoDodanUporabnik = false;
    if (req.query.error == "napakaDodajUporabnika") {
        neuspesnoDodanUporabnik = true;
    }
    //izbris uporabnika
    var successfullyDeleteUser = req.query.deleted;
    var uspesnoZbrisanUporabnik = false;
    if (successfullyDeleteUser == "successfully deleted user") {
        uspesnoZbrisanUporabnik = true;
    }
    //napaka brisanje uporabnika
    var neuspesnoZbrisanUporabnik = false;
    if (req.query.error == "napakaDodajUporabnika") {
        neuspesnoZbrisanUporabnik = true;
    }
    ...
    res.render('users', {
        users: uporabniki,
        usernameC: usernameC,
        successfullyCreateUser: uspesnoDodanUporabnik,
        errorCreateUser: neuspesnoDodanUporabnik,
        successfullyDeleteUser: uspesnoZbrisanUporabnik,
        errorDeleteUser: neuspesnoZbrisanUporabnik
    });
};
...
const shraniUserja = (req, res) => {
    ...
    if (!req.body.name || !req.body.surname || !req.body.username || !req.body.email || !req.body.password || !req.body.password_retype || !req.body.role) {
        ...
        axios({
            ...
            var string = "successfullyAddedUser";
            res.redirect('/users?addUser=' + string);
        }).catch((napaka) => {
            var string = "napakaDodajUporabnika";
            res.redirect('/user-new?error=' + string);
        });
    }
};
...
var podrobnostiUser = (req, res) => {
    ...
    //pododobitev podatkov uporabnika
    var successfullyUpdateUserInfo = req.query.userInfo;
    var uspesnoPosodobljeniPodatkiUserja = false;
    if (successfullyUpdateUserInfo == "succesfullyUpdatedUserInfo") {
        uspesnoPosodobljeniPodatkiUserja = true;
    }
    //napaka pri posodobitvi user info
    var neuspesnoposodobljeniPodatkiUserja = false;
    if (req.query.error == "napakaUpdateInfo") {
        neuspesnoposodobljeniPodatkiUserja = true;
    }
    //posodbitev username uporabnika
    var successfullyUpdateUsername = req.query.username;
    var uspesnoPosodobljenUername = false;
    if (successfullyUpdateUsername == "successfullyChangedUsername") {
        uspesnoPosodobljenUername = true;
    }
    //posodobitev gesla
    var successfullyUpdateUserPassword = req.query.password;
    var uspesnoPosodobljenoGeslo = false;
    if (successfullyUpdateUserPassword == "successfullyChangedPassword") {
        uspesnoPosodobljenoGeslo = true;
    }
    ...
    axios
        ...
        .then((user) => {
            res.render('user-edit', {
                ...
                _id: user.data._id,
                napaka: jeNapaka,
                successfullyUpdateUserInfo: uspesnoPosodobljeniPodatkiUserja,
                errorUpdateUserInfo: neuspesnoposodobljeniPodatkiUserja,
                successfullyUpdateUsername: uspesnoPosodobljenUername,
                successfullyUpdateUserPassword: uspesnoPosodobljenoGeslo
            });
        });
};
...
const posodobiUserja = (req, res) => {
    ...
    if (!req.body.name || !req.body.surname || !req.body.role || !req.body.password) {
        ...
        axios({
            ...
            .then(() => {
                var string = "succesfullyUpdatedUserInfo"
                res.redirect('/users/' + userId + '?userInfo=' + string);
            }).catch((napaka) => {
                var string = "napakaUpdateInfo";
...
const izbrisiUserja = (req, res) => {
    ...
    axios({
        ...
        .then(() => {
            var string = "successfully deleted user"
            res.redirect('/users?deleted=' + string);
        }).catch((napaka) => {
            var string = "errorDeleteUser"
            res.redirect('/users?error=' + string);
            //prikaziNapako(req, res, napaka);
        });
...
const posodobiUsername = (req, res) => {
    ...
    if (!req.body.username) {
        ...
        axios({
            ...
            .then(() => {
                var string = "successfullyChangedUsername"
                res.redirect('/users/' + userId + '?username=' + string);
            }).catch((napaka) => {
                var string = "napaka";
...
const posodobitevGesla = (req, res) => {
    ...
    if (!req.body.username) {
        ...
        axios({
            ...
            .then(() => {
                var string = "successfullyChangedPassword"
                res.redirect('/users/' + userId + '?password=' + string);
            }).catch((napaka) => {
                var string = "napaka";
                res.redirect('/users/' + userId + '/edit-password?error=' + string);
                //prikaziNapako(req, res, napaka);
